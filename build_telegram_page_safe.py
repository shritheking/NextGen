...
new_hero = """<!-- TELEGRAM HERO -->
    <section id="home" class="hero-section" style="padding-top: 140px; padding-bottom: 80px; position: relative;">
      <div class="path-label">~/telegram-bot-development</div>
      
      <div class="hero-grid" style="grid-template-columns: 1.1fr 0.9fr; align-items: center; gap: 40px; display: grid; max-width: 1200px; margin: 0 auto; padding: 0 5%;">
        <div class="hero-text-content">
          <h1 style="font-size: clamp(36px, 5vw, 64px); line-height: 1.1; margin-bottom: 24px;">BUILD → AUTOMATE → SCALE<br><span class="gradient-text">Telegram Bots</span><br>That Actually Do Work.</h1>
          <p class="hero-sub" style="font-size: 18px; color: var(--ink-soft); margin-bottom: 32px; line-height: 1.6;">Custom Telegram bots built for automation, payments, notifications, APIs and business workflows. Your bot. Your workflow. Your infrastructure.</p>
    ...
    <section class="block" id="tech-stack">
      ...
      <div class="filters" style="justify-content: center; margin-top: 30px; display: flex; flex-wrap: wrap; gap: 10px;">
...
# 1. We want to remove EVERYTHING from '<!-- Hero Section -->' to '<!-- Contact Form Section -->'
start_idx = content.find('<!-- Hero Section -->')
end_idx = content.find('<!-- Contact Form Section -->')
if start_idx != -1 and end_idx != -1:
    content = content[:start_idx] + new_hero + '\n    <!-- Contact Form Section -->' + content[end_idx + len('<!-- Contact Form Section -->'):]
    
# Remove the FAQ script that is still lying around between hero and contact form
# Wait, if we wiped EVERYTHING between Hero and Contact, the FAQ script was ALREADY WIPED OUT!
# Because the FAQ script was between the FAQ section and the Contact Form section!
# This is beautifully simple and flawless.

with open("telegram-bot-development.html", "w", encoding="utf-8") as f:
    f.write(content)

print("Created telegram-bot-development.html correctly this time.")
