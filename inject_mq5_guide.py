...
# Revert ea_customization_guide
start_idx = content.find("ea_customization_guide: {")
if start_idx != -1:
    end_idx = content.find("  },", start_idx) + 4 # skip past },
...
# Clean out any stray commas to be safe, find insertion point
index = content.find("const templateSelector = document.getElementById")
brace_index = content.rfind("};", 0, index)

# Avoid comma issues
content = content[:brace_index].rstrip().rstrip(",") + "\n"
...
print("Reverted Customization Guide and Added MQ5 Guide to documents.js")
